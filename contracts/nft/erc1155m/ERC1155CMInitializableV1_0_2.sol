...
/// @dev Self-contained implementation with Creator Token functionality for use with upgradeable proxies
    ...
    AutomaticValidatorTransferApproval
{
    ...
    /// @notice Initializes the contract with Creator Token functionality
    ...
    function totalMintedByAddress(address account) public view override returns (uint256[] memory) {
        ...
        returns (bool)
    {
        return interfaceId == type(ICreatorToken).interfaceId || super.supportsInterface(interfaceId)
            || ERC2981.supportsInterface(interfaceId) || ERC1155Upgradeable.supportsInterface(interfaceId);
    }
    ...
    function setMaxMintableSupply(uint256 tokenId, uint256 maxMintableSupply) external onlyOwner {
    ...
    function _totalMintedByTokenByAddress(address account, uint256 tokenId) internal view returns (uint256) {
    ...
    function _totalMintedByStage(uint256 stage) internal view returns (uint256[] memory) {
        uint256[] memory totalMinted = new uint256[](_numTokens);
        for (uint256 token = 0; token < _numTokens; token++) {
    ...
    /// @dev Overrides the _beforeTokenTransfer function to add custom logic and Creator Token validation
